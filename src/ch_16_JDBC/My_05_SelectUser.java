...
class DBSelect extends DB {
	...
	protected ResultSet querySelect(String sql) {
		...
		try {
			...
			// SQL문이 INSERT, UPDATE, DELTE 문일 경우네는 executeUpadt() 메서드를 호출하지만
			// 데이터를 가져오는 SELCT 문일 경우네는 executeQuery() 메서드를 호출
			resultSet = preparedStatement.executeQuery();
		} catch(SQLException e) {
			System.out.println("error: " + e);
		}
		// resultSet은 SELECT 문에 기술된 칼럼으로 구성된 행row의 집합
		return resultSet;
	}
	
	private void printUser(String sql) throws SQLException {
		// select 실행 결과를 받아 콜솔에 출력
		// 전체 목록 출현과 한 열만 출력하는 기능
		int totalRow;
		int num = 1;
		ResultSet resultSet = querySelect(sql);
		
		// ResultSet의 개수를 구해서 1일 경우와 1이 아닐 경우를 불러
		// ResultSet의 개수를 구하는 메서드는 없어서 다른 방향으로 개수를 구함
		...
		if (totalRow == 1) {
			...
			// 한줄 한줄 내려가며 데이터를 가져오고 데이터가 끝까지 가면 종료
			while (resultSet.next()) {
				System.out.println(num + "번쨰 회원");
				System.out.println("아이디 : " + resultSet.getString(1));
				System.out.println("이름 : " + resultSet.getString(2));
				System.out.println("나이 : " + resultSet.getString(3));
				System.out.println("직업 : " + resultSet.getString(4));
	...
	public void getOneUser() throws SQLException {
		...
		do {
			...
			System.out.println("검색할 회원의 아이디가 " + userID + "이 맞습습니까? (y/n) ");
